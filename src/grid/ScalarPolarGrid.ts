...
import { IPolarParams, Polar } from './Polar'

export interface IScalarPolarGrid extends IPolarParams {
    reverseX?: boolean
    reverseY?: boolean
    zs?: number[]
}
 ...
 * Scalar Field
 */
export class ScalarPolarGrid extends Polar<number> {

    public static fromData(def: IScalarPolarGrid, datos: Array<{ [x: string]: number }>, nomeVar: string = 'c'): ScalarPolarGrid {
        const values: number[] = datos.map((it: { [x: string]: number }) => it[nomeVar])
        return new ScalarPolarGrid({ ...def, zs: values })
    }

    public static fromArray(def: IScalarPolarGrid, values: number[]): ScalarPolarGrid {
        return new ScalarPolarGrid({ ...def, zs: values })
    }
    ...
    constructor(params: IScalarPolarGrid) {
        super(params)
